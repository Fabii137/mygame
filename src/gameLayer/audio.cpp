...
void update(float dt) {
	...
	if (currentTransitionMusic != Musics::NoneMusic) {
		...
		float t { std::clamp(
			  musicTransitionTime / MUSIC_TRANSITION_DURATION, 0.f, 1.f) };

		SetMusicVolume(
		    allMusic[currentMusicPlaying], std::lerp(0.f, targetVolume, 1.f - t));
		SetMusicVolume(
		    allMusic[currentTransitionMusic], std::lerp(0.f, targetVolume, t));
...
void playSound(int sound, float volume) {
	...
	permaAssertComment(
	    !variations.empty(), "No sounds variations loaded for type");
