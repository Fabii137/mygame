...
void Background::draw(
    AssetManager& assetManager, Camera2D camera, Vector2 mapSize) {
...
void Background::updateDayNight(float dt) {
	...
	if (phase >= sunriseStart && phase < sunriseEnd) {
		m_Transition = 1.f
		    - std::clamp(
		        (phase - sunriseStart) / (sunriseEnd - sunriseStart), 0.f, 1.f);
	} else if (phase >= dayStart && phase < dayEnd) {
		m_Transition = 0.f;
	} else if (phase >= sunsetStart && phase < sunsetEnd) {
		m_Transition = std::clamp(
		    (phase - sunsetStart) / (sunsetEnd - sunsetStart), 0.f, 1.f);
	} else {
		m_Transition = 1.f;
	}
}
...
void Background::drawDayNightBg(AssetManager& assetManager) {
	...
	DrawTexturePro(
	    assetManager.nightSky, skySrc, skyDest, {}, 0.f, Fade(WHITE, nightBlend));

	float starAlpha { std::clamp(nightBlend * 1.1f, 0.f, 1.f) };
	if (starAlpha > 0.05f) {
		DrawTexturePro(
		    assetManager.stars, skySrc, skyDest, {}, 0.f, Fade(WHITE, starAlpha));
	}
	...
	float sunWidth { static_cast<float>(assetManager.sun.width)
		* celestialScale };
	float sunHeight { static_cast<float>(assetManager.sun.height)
		* celestialScale };
	float moonWidth { static_cast<float>(assetManager.moon.width)
		* celestialScale };
	...
	DrawTexturePro(assetManager.sun,
	    ...
	    sunRect, {}, 0.f, Fade(WHITE, std::clamp(daylight * 1.2f, 0.f, 1.f)));
	...
	DrawTexturePro(assetManager.moon,
	    ...
	    moonRect, {}, 0.f, Fade(WHITE, std::clamp(nightBlend * 1.2f, 0.f, 1.f)));
