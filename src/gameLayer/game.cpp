...
void Game::updateEntities(float dt) {
  ...
  for (auto it{m_Entities.entities.begin()}; it != m_Entities.entities.end();) {
    ...
    float distanceToPlayer{
        Vector2Distance(entity->position(), m_Player.position())};
    ...
    bool shouldKill{!it->second->update(dt, updateData) ||
                    it->second->getHealth() <= 0.f || !inBounds ||
                    shouldDespawn};
    if (shouldKill) {
...
std::optional<Vector2> Game::findGroundSpawnPosition(float x) {
  ...
  int endY{std::min(m_GameMap.h - 2,
                    playerY + static_cast<int>(EnemySpawnMaxDistance))};
...
void Game::updateStructureSelection() {
  ...
  if (IsKeyPressed(KEY_THREE)) {
    m_CopyStructure.pasteIntoMap(
        m_GameMap,
        {static_cast<float>(hoveredCell.x), static_cast<float>(hoveredCell.y)});
  }
...
int Game::getTreeAtlasColumn(int x, int y) {
  ...
  bool groundBottom{bottom && bottom->type != Block::Air &&
                    bottom->type != Block::WoodLog &&
                    bottom->type != Block::Leaves};
...
void Game::renderImGuiWindows() {
  ...
  if (ImGui::CollapsingHeader("Creative")) {
    ...
    auto addTexButton = [&](int i, std::function<void(void)> onClick) {
      auto atlas{
          getTextureAtlas(i, 0, Constants::CellSize, Constants::CellSize)};
    ...
    if (ImGui::CollapsingHeader("Walls")) {
      for (std::uint16_t i{Block::BLOCKS_COUNT}; i < Wall::WALLS_COUNT; i++) {
        addTexButton(
  ...
  if (ImGui::CollapsingHeader("World Generation")) {
    ...
    ImGui::SliderInt("Mountain Octaves", &m_WorldSettings.mountain.octaves, 1,
                     8);
    ImGui::SliderInt("Mountain Height Min",
                     &m_WorldSettings.mountainHeightStart, 0, 250);
    ImGui::SliderInt("Mountain Height Max", &m_WorldSettings.mountainHeightEnd,
                     0, 300);
    ImGui::Separator();

    ImGui::Text("Plains Noise");
    ImGui::SliderFloat("Plains Frequency", &m_WorldSettings.plains.frequency,
                       0.001f, 0.1f);
    ImGui::SliderInt("Plains Octaves", &m_WorldSettings.plains.octaves, 1, 8);
    ImGui::SliderInt("Plains Height Min", &m_WorldSettings.plainsHeightStart, 0,
                     250);
    ImGui::SliderInt("Plains Height Max", &m_WorldSettings.plainsHeightEnd, 0,
                     300);
    ImGui::Separator();

    ImGui::Text("Blend Noise");
    ImGui::SliderFloat("Blend Frequency", &m_WorldSettings.blend.frequency,
                       0.0001f, 0.05f);
    ImGui::SliderInt("Blend Octaves", &m_WorldSettings.blend.octaves, 1, 8);
    ImGui::Separator();

    ImGui::Text("Stone Noise");
    ImGui::SliderFloat("Stone Frequency", &m_WorldSettings.stone.frequency,
                       0.001f, 0.1f);
    ImGui::SliderInt("Stone Octaves", &m_WorldSettings.stone.octaves, 1, 8);
    ImGui::SliderInt("Stone Offset Min", &m_WorldSettings.stoneOffsetStart,
                     -100, 100);
    ImGui::SliderInt("Stone Offset Max", &m_WorldSettings.stoneOffsetEnd, -100,
                     100);
    ImGui::Separator();

    ImGui::Text("Cave Noise");
    ImGui::SliderFloat("Cave1 Frequency", &m_WorldSettings.cave1.frequency,
                       0.001f, 0.1f);
    ImGui::SliderInt("Cave1 Octaves", &m_WorldSettings.cave1.octaves, 1, 8);
    ImGui::SliderFloat("Cave2 Frequency", &m_WorldSettings.cave2.frequency,
                       0.001f, 0.1f);
    ImGui::SliderInt("Cave2 Octaves", &m_WorldSettings.cave2.octaves, 1, 8);
    ImGui::SliderFloat("Min Cave Noise", &m_WorldSettings.minCaveNoise, 0.f,
                       1.f);
  }
  ImGui::Separator();
