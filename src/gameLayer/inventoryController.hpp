...
public:
	void update(const AssetManager& assetManager, InventoryContext& ctx) {
...
private:
	...
	void drop(
		...
		m_DragState.draggedItem = targetCtx.inventory.insert(
		    targetSlot, m_DragState.draggedItem, single);
		if (m_DragState.draggedItem.empty()) {
			cancelDrag();
		}
	}

	void cancelDrag() { m_DragState = {}; }
