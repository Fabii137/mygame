...
void PhysicalEntity::checkCollisionOnce(Vector2 &pos, GameMap &mapData) {
  Vector2 delta{pos - lastPosition};
  Vector2 newPos{performCollisionOnOneAxis(mapData, {pos.x, lastPosition.y},
                                           {delta.x, 0.f})};

  pos = performCollisionOnOneAxis(mapData, {newPos.x, pos.y}, {0.f, delta.y});
}

Vector2 PhysicalEntity::performCollisionOnOneAxis(GameMap &mapData, Vector2 pos,
                                                  Vector2 delta) {
