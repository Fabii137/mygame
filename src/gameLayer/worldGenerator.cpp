...
NoiseData::NoiseData(float *mountain, float *plains, float *blend, float *stone,
    ...
    : mountain(mountain), plains(plains), blend(blend), stone(stone),
...
WorldGenerator::WorldGenerator(GameMap &gameMap, const WorldSettings &settings,
                               std::uint32_t seed)
    : m_GameMap(gameMap), m_Settings(settings), m_Seed(seed) {}
  ...
  for (int x{}; x < WIDTH; x++) {
    ...
    float plainsHeight{
        std::lerp(static_cast<float>(m_Settings.plainsHeightStart),
                  static_cast<float>(m_Settings.plainsHeightEnd),
                  noiseData.plains[x]),
    };
...
void WorldGenerator::generateBiomeMap(TerrainData &terrainData,
...
const Biome *WorldGenerator::getBiomeAt(const TerrainData &terrainData,
                                        int x) const {
...
bool WorldGenerator::isBiomeAllowed(const std::vector<Biome::Type> &allowed,
                                    const Biome *biome) {
  if (!biome) {
    return false;
  }
...
void WorldGenerator::generateDesertColumn(int x, const Biome &biome,
  ...
  float desertDistance{1 - distanceFromDesertMid /
                               static_cast<float>(desertHalfWidth)};
...
void WorldGenerator::generateSnowColumn(int x, const Biome &biome,
                                        const TerrainData &terrainData) {
...
void WorldGenerator::generateCaves(const NoiseData &noiseData) {
  ...
  auto getCaveNoise = [](const float *noise, int x, int y) {
...
void WorldGenerator::generateWorms(std::ranlux24_base &rng) {
  auto spawnWorm = [&](Vector2 start, Vector2 direction, int length,
                       float radius) {
    float x{start.x};
    float y{start.y};
...
void WorldGenerator::generateStructures(const TerrainData &terrainData,
                                        std::ranlux24_base &rng) {
  for (const Biome &biome : terrainData.biomes) {
    for (int x{biome.startX}; x < biome.endX; x++) {
      ...
      const std::string &variant{
          selected
              ->variants[getRandomInt(rng, 0, selected->variants.size() - 1)],
      };
...
void WorldGenerator::generateOres(const TerrainData &terrainData,
                                  std::ranlux24_base &rng) {
...
NoiseData WorldGenerator::generateNoiseData() {
  auto generateNoise = [&](const NoiseSettings &noiseSettings,
